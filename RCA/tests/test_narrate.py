...
MINIMAL_LEDGER_WITH_FINDINGS = {
    ...
    "findings": [{
        ...
        "candidates": [], "verdict": "broad_based", "ruled_out": [],
    }],
    "verdict": "broad_based",
}


def test_narrate_uses_template_when_alert_did_not_reproduce():
    ledger = {"metric_id": "revenue", "window": {"start": "a", "end": "b"}, "verdict": "not_reproducible"}
    result = narrate(ledger)
    assert result["source"] == "template"
    assert "not_reproducible" in result["narrative"]


def test_narrate_falls_back_to_template_without_a_gemini_key():
    ...
    result = narrate(MINIMAL_LEDGER_WITH_FINDINGS)
    assert result["source"] == "template"
