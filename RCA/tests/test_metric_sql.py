...
ADDITIVE = {**RATIO, "sql": "sum(revenue)", "numerator": "sum(revenue)",
             "denominator": "", "is_ratio": 0, "detector": "robust_z",
             "min_samples": 2000, "min_effect_rel": 0.03, "min_effect_abs": 0.0}


def _sql(meta, dims=("ALL",)):
    return metric_sql.deviation_sql(meta, list(dims), "{h:DateTime}", "{s:DateTime}", "{e:DateTime}")
...
def test_baseline_partitions_on_hour_of_day_and_day_type():
    ...
    assert "PARTITION BY dim_name, dim_value, toHour(ts), toDayOfWeek(ts) >= 6" in _sql(RATIO)


def test_dimension_fanout_is_one_query_over_every_cut():
    sql = _sql(RATIO, ["ALL", "os_version", "country"])
    assert "ARRAY JOIN [('ALL', ''), ('os_version', toString(os_version)), " \
...
def test_value_sql_reuses_the_same_formula_under_a_different_where():
    sql = metric_sql.value_sql(RATIO, "event_time > {s:DateTime} AND NOT (os_version = 'x')")
