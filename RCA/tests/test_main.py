...
def test_global_series_unknown_metric_returns_404():
    with patch("app.main.get_metric", return_value=None):
        resp = client.post("/internal/global-series", json={"metric_id": "not_real", **SERIES_WINDOW})
    assert resp.status_code == 404


def test_global_series_known_metric_returns_reproduce_global_rows():
    rows = [{"ts": "2026-07-30T09:00:00", "actual": 0.75, "expected": 0.78,
              "z_score": -4.1, "delta_rel": -0.03, "is_anomaly": 1}]
    with patch("app.main.get_metric", return_value={"metric_id": "fill_rate"}), \
         patch("app.main.reproduce_global", return_value=rows) as mocked:
        resp = client.post("/internal/global-series", json={"metric_id": "fill_rate", **SERIES_WINDOW})
...
def test_global_series_nan_from_insufficient_baseline_becomes_json_null():
    ...
    rows = [{"ts": "2026-07-30T09:00:00", "actual": 7769.0, "expected": float("nan"),
              "z_score": float("nan"), "delta_rel": float("nan"), "is_anomaly": 0}]
    with patch("app.main.get_metric", return_value={"metric_id": "requests"}), \
         patch("app.main.reproduce_global", return_value=rows):
        resp = client.post("/internal/global-series", json={"metric_id": "requests", **SERIES_WINDOW})
    assert resp.status_code == 200
    assert resp.json() == [{"ts": "2026-07-30T09:00:00", "actual": 7769.0, "expected": None,
                              "z_score": None, "delta_rel": None, "is_anomaly": 0}]
...
def test_segment_series_invalid_dim_name_returns_400():
    with patch("app.main.get_metric", return_value={"metric_id": "fill_rate"}), \
         patch("app.main.reproduce_segment", side_effect=ValueError("unknown dimension column 'nope'")):
        resp = client.post(
            "/internal/segment-series",
...
def test_segment_series_filters_to_requested_dim_values():
    rows = [
        {"ts": "2026-07-30T09:00:00", "dim_value": "Android 15", "actual": 0.43, "expected": 0.74},
        {"ts": "2026-07-30T09:00:00", "dim_value": "Android 14", "actual": 0.77, "expected": 0.78},
    ]
    with patch("app.main.get_metric", return_value={"metric_id": "fill_rate"}), \
         patch("app.main.reproduce_segment", return_value=rows):
        resp = client.post(
            "/internal/segment-series",
            json={"metric_id": "fill_rate", "dim_name": "os_version",
                   "dim_values": ["Android 15"], **SERIES_WINDOW},
        )
